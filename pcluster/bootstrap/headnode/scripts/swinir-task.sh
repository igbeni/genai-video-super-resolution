...
local_input_file="${src_dir}/${base_name}_${task_frame}.${image_type}"
local_output_file="${dest_dir}/${base_name}_${task_frame}.${image_type}"
...
if [ -f "${src_dir}/global_cache_dir" ]; then
    GLOBAL_CACHE_DIR=$(cat "${src_dir}/global_cache_dir")
    MODEL_CACHE_DIR=$(cat "${src_dir}/model_cache_dir")
    FRAME_CACHE_DIR=$(cat "${src_dir}/frame_cache_dir")
    TASK_CACHE_DIR=$(cat "${src_dir}/task_cache_dir")
    ...
    export SWINIR_MODEL_CACHE="${MODEL_CACHE_DIR}"

    # Create symlink to model cache for the inference endpoint
    if [ ! -L "/tmp/swinir_models" ] && [ -d "${MODEL_CACHE_DIR}" ]; then
        ln -sf "${MODEL_CACHE_DIR}" "/tmp/swinir_models"
    fi
else
    # Fall back to local cache
    mkdir -p /tmp/swinir_cache

    # Define cache paths using local cache
    cache_input_file="/tmp/swinir_cache/${base_name}_${task_frame}.${image_type}"
    cache_metadata_file="/tmp/swinir_cache/${base_name}_${task_frame}.meta"
fi
...
# Process the frame
payload="{ \"input_file_path\" : \"${local_input_file}\", \"output_file_path\" : \"${local_output_file}\", \"job_id\" : \"1234\", \"batch_id\" : \"01\" }"
